...
(function(process) {
  ...
  !function(global, factory) {
  ...
  }(this, function(exports, _angular_core, rxjs_Subject, rxjs_observable_PromiseObservable, rxjs_operator_toPromise) {
    "use strict";
    ...
    function isWhitespace(code) {
      return code >= $TAB && $SPACE >= code || code == $NBSP;
    }
    function isIdentifierStart(code) {
      return code >= $a && $z >= code || code >= $A && $Z >= code || code == $_ || code == $$;
    }
    ...
    function isIdentifierPart(code) {
      return code >= $a && $z >= code || code >= $A && $Z >= code || code >= $0 && $9 >= code || code == $_ || code == $$;
    }
    ...
    function tokenizeHtml(sourceContent, sourceUrl, tokenizeExpansionForms) {
      return void 0 === tokenizeExpansionForms && (tokenizeExpansionForms = !1), new _HtmlTokenizer(new ParseSourceFile(sourceContent, sourceUrl), tokenizeExpansionForms).tokenize();
    }
    ...
    function isWhitespace$1(code) {
      return code >= $TAB$1 && $SPACE$1 >= code || code === $NBSP$1;
    }
    function isNameEnd(code) {
      return isWhitespace$1(code) || code === $GT$1 || code === $SLASH$1 || code === $SQ$1 || code === $DQ$1 || code === $EQ$1;
    }
    function isPrefixEnd(code) {
      return ($a$1 > code || code > $z$1) && ($A$1 > code || code > $Z$1) && ($0$1 > code || code > $9$1);
    }
    ...
    function isAsciiHexDigit(code) {
      return code >= $a$1 && $f$1 >= code || code >= $A$1 && $F >= code || code >= $0$1 && $9$1 >= code;
    }
    function compareCharCodeCaseInsensitive(code1, code2) {
      return toUpperCaseCharCode(code1) == toUpperCaseCharCode(code2);
    }
    function toUpperCaseCharCode(code) {
      return code >= $a$1 && $z$1 >= code ? code - $a$1 + $A$1 : code;
    }
    ...
    function stripComments(input) {
      return StringWrapper.replaceAllMapped(input, _commentRe, function() {
        return "";
      });
    }
    ...
    function getStylesVarName(component) {
      var result = "styles";
      return isPresent(component) && (result += "_" + component.type.name), result;
    }
    ...
    function _mergeAnimationStyles(stylesList, newItem) {
      if (isStringMap(newItem) && stylesList.length > 0) {
        ...
        if (isStringMap(lastItem))
          return stylesList[lastIndex] = StringMapWrapper.merge(lastItem, newItem), void 0;
      }
    ...
    function _parseAnimationKeyframes(keyframeSequence, currentTime, collectedStyles, stateStyles, errors) {
      ...
      }), totalOffsets > 0 && totalEntries > totalOffsets && (errors.push(new AnimationParseError("Not all style() entries contain an offset for the provided keyframe()")), totalOffsets = totalEntries);
      ...
      keyframeSequence.steps.forEach(function(styleMetadata) {
        ...
        }), isPresent(offset) ? doSortKeyframes = doSortKeyframes || lastOffset > offset : offset = index == limit ? _TERMINAL_KEYFRAME : margin * index, rawKeyframes.push([offset, keyframeStyles]), lastOffset = offset, index++;
      ...
      if (entry instanceof CompileAnimationWithStepsMetadata) {
        ...
        if (entry.steps.forEach(function(entry) {
          ...
          if (entry instanceof CompileAnimationStyleMetadata)
            ...
            }), previousStyles = entry.styles, void 0;
    ...
    function createQueryList(query, directiveInstance, propertyName, compileView) {
      ...
      return compileView.createMethod.addStmt(THIS_EXPR.prop(propertyName).set(importExpr(Identifiers.QueryList).instantiate([])).toStmt()), expr;
    }
    ...
    function bindDirectiveInputs(directiveAst, directiveInstance, compileElement) {
      if (0 !== directiveAst.inputs.length) {
        ...
        var lifecycleHooks = directiveAst.directive.lifecycleHooks,
            calcChangesMap = -1 !== lifecycleHooks.indexOf(LifecycleHooks.OnChanges),
    ...
    function createViewClass(view, renderCompTypeVar, nodeDebugInfosVar) {
      var viewConstructorArgs = [new FnParam(ViewConstructorVars.viewUtils.name, importType(Identifiers.ViewUtils)), new FnParam(ViewConstructorVars.parentInjector.name, importType(Identifiers.Injector)), new FnParam(ViewConstructorVars.declarationEl.name, importType(Identifiers.AppElement))],
          superConstructorArgs = [variable(view.className), renderCompTypeVar, ViewTypeEnum.fromValue(view.viewType), ViewConstructorVars.viewUtils, ViewConstructorVars.parentInjector, ViewConstructorVars.declarationEl, ChangeDetectionStrategyEnum.fromValue(getChangeDetectionMode(view))];
    ...
    function getChangeDetectionMode(view) {
      var mode;
      return mode = view.viewType === ViewType.COMPONENT ? isDefaultChangeDetectionStrategy(view.component.changeDetection) ? _angular_core.ChangeDetectionStrategy.CheckAlways : _angular_core.ChangeDetectionStrategy.CheckOnce : _angular_core.ChangeDetectionStrategy.CheckAlways;
    }
    ...
    function partition(nodes, errors, implicitTags) {
      for (var parts = [],
        ...
        if (_isOpeningComment(n)) {
          ...
          for (i++; !_isClosingComment(nodes[i]); )
            if (temp.push(nodes[i++]), i === nodes.length) {
              errors.push(new I18nError(n.sourceSpan, "Missing closing 'i18n' comment."));
              break;
            }
          parts.push(new Part(null, null, temp, i18n, !0));
        } else if (n instanceof HtmlElementAst) {
          var i18n = _findI18nAttr(n),
              hasI18n = isPresent(i18n) || implicitTags.indexOf(n.name) > -1;
          parts.push(new Part(n, null, n.children, isPresent(i18n) ? i18n.value : null, hasI18n));
        } else
          n instanceof HtmlTextAst && parts.push(new Part(null, n, null, null, !1));
      }
      return parts;
    }
    ...
    function expandNodes(nodes) {
      var e = new _Expander,
          n = htmlVisitAll(e, nodes);
      return new ExpansionResult(n, e.expanded, e.errors);
    }
    function _expandPluralForm(ast, errors) {
      var children = ast.cases.map(function(c) {
        -1 != PLURAL_CASES.indexOf(c.value) || c.value.match(/^=\d+$/) || errors.push(new I18nError(c.valueSourceSpan, 'Plural cases should be "=<number>" or one of ' + PLURAL_CASES.join(", ")));
    ...
    function _expandDefaultForm(ast) {
      var children = ast.cases.map(function(c) {
        var expansionResult = expandNodes(c.expression),
            i18nAttrs = expansionResult.expanded ? [] : [new HtmlAttrAst("i18n", ast.type + "_" + c.value, c.valueSourceSpan)];
      ...
      nodes.forEach(function(item) {
        if (item instanceof HtmlElementAst) {
          ...
          if (msg.name != _MSG_ELEMENT)
            return errors.push(new XmbDeserializationError(item.sourceSpan, 'Unexpected element "' + msg.name + '"')), void 0;
          var id_1 = _id(msg);
          if (isBlank(id_1))
            return errors.push(new XmbDeserializationError(item.sourceSpan, '"' + _ID_ATTR + '" attribute is missing')), void 0;
    ...
    var STRING_MAP_PROTO = Object.getPrototypeOf({}),
        ...
        ProviderAst = function() {
          ...
          return ProviderAst.prototype.visit = function() {
            return null;
          }, ProviderAst;
        }();
    ...
    var TokenType,
        ...
        ChangeDetectorState = _angular_core.__core_private__.ChangeDetectorState,
        ...
        ListWrapper = function() {
          ...
          return ListWrapper.createFixedSize = function(size) {
            ...
            return index > -1 ? (list.splice(index, 1), !0) : !1;
        ...
        RecursiveAstVisitor = function() {
          ...
          return RecursiveAstVisitor.prototype.visitBinary = function(ast) {
          ...
          }, RecursiveAstVisitor.prototype.visitImplicitReceiver = function() {
            return null;
          ...
          }, RecursiveAstVisitor.prototype.visitLiteralPrimitive = function() {
            return null;
          ...
          }, RecursiveAstVisitor.prototype.visitQuote = function() {
            return null;
          }, RecursiveAstVisitor;
        }();
    !function(TokenType) {
      ...
      return Token.prototype.isCharacter = function(code) {
        ...
        _Scanner = function() {
          ...
          return _Scanner.prototype.advance = function() {
            ...
            return SetWrapper.has(KEYWORDS, str) ? newKeywordToken(start, str) : newIdentifierToken(start, str);
    ...
    var KEYWORDS = SetWrapper.createFromList(["var", "let", "null", "undefined", "true", "false", "if", "else"]),
        ...
        Parser = function() {
          ...
          return Parser.prototype.parseAction = function(input, location) {
            ...
            for (var errLocation = "",
                j = 0; partInErrIdx > j; j++)
              errLocation += 0 === j % 2 ? parts[j] : "{{" + parts[j] + "}}";
    ...
    var HtmlTagContentType,
        _ParseAST = function() {
          ...
          return _ParseAST.prototype.peek = function(offset) {
            ...
            return this.next.isCharacter(code) ? (this.advance(), !0) : !1;
            ...
            return this.next.isOperator(op) ? (this.advance(), !0) : !1;
 ...
 while (operatorFound);
        ...
        SimpleExpressionChecker = function() {
          ...
          return SimpleExpressionChecker.check = function(ast) {
          ...
          }, SimpleExpressionChecker.prototype.visitImplicitReceiver = function() {}, SimpleExpressionChecker.prototype.visitInterpolation = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitLiteralPrimitive = function() {}, SimpleExpressionChecker.prototype.visitPropertyRead = function() {}, SimpleExpressionChecker.prototype.visitPropertyWrite = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitSafePropertyRead = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitMethodCall = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitSafeMethodCall = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitFunctionCall = function() {
            this.simple = !1;
          ...
          }, SimpleExpressionChecker.prototype.visitBinary = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitPrefixNot = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitConditional = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitPipe = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitKeyedRead = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitKeyedWrite = function() {
            this.simple = !1;
            ...
            return res;
          }, SimpleExpressionChecker.prototype.visitChain = function() {
            this.simple = !1;
          }, SimpleExpressionChecker.prototype.visitQuote = function() {
            this.simple = !1;
        ...
        ParseError = function() {
          ...
          return ParseError.prototype.toString = function() {
            ...
            for (var ctxEnd = ctxStart,
                ctxLen = 0,
                ctxLines = 0; 100 > ctxLen && ctxStart > 0 && (ctxStart--, ctxLen++, "\n" != source[ctxStart] || 3 != ++ctxLines); )
              ;
            for (ctxLen = 0, ctxLines = 0; 100 > ctxLen && ctxEnd < source.length - 1 && (ctxEnd++, ctxLen++, "\n" != source[ctxEnd] || 3 != ++ctxLines); )
              ;
      ...
      return HtmlToken;
        ...
        _HtmlTokenizer = function() {
          function _HtmlTokenizer(file, tokenizeExpansionForms) {
            this.file = file, this.tokenizeExpansionForms = tokenizeExpansionForms, this._peek = -1, this._nextPeek = -1, this._index = -1, this._line = 0, this._column = -1, this._expansionCaseStack = [], this.tokens = [], this.errors = [], this._input = file.content, this._length = file.content.length, this._advance();
          }
          return _HtmlTokenizer.prototype._processCarriageReturns = function(content) {
            ...
            for (; this._peek !== $EOF$1; ) {
              var start = this._getLocation();
              try {
                this._attemptCharCode($LT$1) ? this._attemptCharCode($BANG$1) ? this._attemptCharCode($LBRACKET$1) ? this._consumeCdata(start) : this._attemptCharCode($MINUS$1) ? this._consumeComment(start) : this._consumeDocType(start) : this._attemptCharCode($SLASH$1) ? this._consumeTagClose(start) : this._consumeTagOpen(start) : isExpansionFormStart(this._peek, this._nextPeek) && this.tokenizeExpansionForms ? this._consumeExpansionFormStart() : isExpansionCaseStart(this._peek) && this._isInExpansionForm() && this.tokenizeExpansionForms ? this._consumeExpansionCaseStart() : this._peek === $RBRACE$1 && this._isInExpansionCase() && this.tokenizeExpansionForms ? this._consumeExpansionCaseEnd() : this._peek === $RBRACE$1 && this._isInExpansionForm() && this.tokenizeExpansionForms ? this._consumeExpansionFormEnd() : this._consumeText();
              } catch (e) {
            ...
            this._peek === $LF$1 ? (this._line++, this._column = 0) : this._peek !== $LF$1 && this._peek !== $CR$1 && this._column++, this._index++, this._peek = this._index >= this._length ? $EOF$1 : StringWrapper.charCodeAt(this._input, this._index), this._nextPeek = this._index + 1 >= this._length ? $EOF$1 : StringWrapper.charCodeAt(this._input, this._index + 1);
          }, _HtmlTokenizer.prototype._attemptCharCode = function(charCode) {
            return this._peek === charCode ? (this._advance(), !0) : !1;
          }, _HtmlTokenizer.prototype._attemptCharCodeCaseInsensitive = function(charCode) {
            return compareCharCodeCaseInsensitive(this._peek, charCode) ? (this._advance(), !0) : !1;
            ...
            for (var indexBeforeAttempt = this._index,
                columnBeforeAttempt = this._column,
                lineBeforeAttempt = this._line,
                i = 0; i < chars.length; i++)
              if (!this._attemptCharCode(StringWrapper.charCodeAt(chars, i)))
                return this._index = indexBeforeAttempt, this._column = columnBeforeAttempt, this._line = lineBeforeAttempt, !1;
            return !0;
            ...
            var start = this._getLocation();
            if (this._advance(), !this._attemptCharCode($HASH$1)) {
              var startPosition = this._savePosition();
              if (this._attemptCharCodeUntilFn(isNamedEntityEnd), this._peek != $SEMICOLON$1)
            ...
            if (this._attemptCharCodeUntilFn(isDigitEntityEnd), this._peek != $SEMICOLON$1)
            ...
            var _this = this;
            this._beginToken(HtmlTokenType.COMMENT_START, start), this._requireCharCode($MINUS$1), this._endToken([]);
            var textToken = this._consumeRawText(!1, $MINUS$1, function() {
            ...
            var textToken = this._consumeRawText(!1, $RBRACKET$1, function() {
              return _this._attemptStr("]>");
            });
            this._beginToken(HtmlTokenType.CDATA_END, textToken.sourceSpan.end), this._endToken([]);
          }, _HtmlTokenizer.prototype._consumeDocType = function(start) {
            this._beginToken(HtmlTokenType.DOC_TYPE, start), this._attemptUntilChar($GT$1), this._advance(), this._endToken([this._input.substring(start.offset + 2, this._index - 1)]);
          }, _HtmlTokenizer.prototype._consumePrefixAndName = function() {
            for (var nameOrPrefixStart = this._index,
                prefix = null; this._peek !== $COLON$1 && !isPrefixEnd(this._peek); )
              this._advance();
            var nameStart;
            this._peek === $COLON$1 ? (this._advance(), prefix = this._input.substring(nameOrPrefixStart, this._index - 1), nameStart = this._index) : nameStart = nameOrPrefixStart, this._requireCharCodeUntilFn(isNameEnd, this._index === nameStart ? 1 : 0);
            ...
            try {
              ...
              for (this._consumeTagOpenStart(start), lowercaseTagName = this._input.substring(nameStart, this._index).toLowerCase(), this._attemptCharCodeUntilFn(isNotWhitespace); this._peek !== $SLASH$1 && this._peek !== $GT$1; )
                this._consumeAttributeName(), this._attemptCharCodeUntilFn(isNotWhitespace), this._attemptCharCode($EQ$1) && (this._attemptCharCodeUntilFn(isNotWhitespace), this._consumeAttributeValue()), this._attemptCharCodeUntilFn(isNotWhitespace);
              this._consumeTagOpenEnd();
            } catch (e) {
              if (e instanceof ControlFlowError)
                return this._restorePosition(savedPos), this._beginToken(HtmlTokenType.TEXT, start), this._endToken(["<"]), void 0;
              throw e;
            }
            ...
            var _this = this,
                textToken = this._consumeRawText(decodeEntities, $LT$1, function() {
                  return _this._attemptCharCode($SLASH$1) ? (_this._attemptCharCodeUntilFn(isNotWhitespace), _this._attemptStrCaseInsensitive(lowercaseTagName) ? (_this._attemptCharCodeUntilFn(isNotWhitespace), _this._attemptCharCode($GT$1) ? !0 : !1) : !1) : !1;
                });
            ...
            this._beginToken(tokenType), this._requireCharCode($GT$1), this._endToken([]);
          }, _HtmlTokenizer.prototype._consumeTagClose = function(start) {
            this._beginToken(HtmlTokenType.TAG_CLOSE, start), this._attemptCharCodeUntilFn(isNotWhitespace);
            var prefixAndName = this._consumePrefixAndName();
            this._attemptCharCodeUntilFn(isNotWhitespace), this._requireCharCode($GT$1), this._endToken(prefixAndName);
          }, _HtmlTokenizer.prototype._consumeExpansionFormStart = function() {
            this._beginToken(HtmlTokenType.EXPANSION_FORM_START, this._getLocation()), this._requireCharCode($LBRACE$1), this._endToken([]), this._beginToken(HtmlTokenType.RAW_TEXT, this._getLocation());
            var condition = this._readUntil($COMMA$1);
            this._endToken([condition], this._getLocation()), this._requireCharCode($COMMA$1), this._attemptCharCodeUntilFn(isNotWhitespace), this._beginToken(HtmlTokenType.RAW_TEXT, this._getLocation());
            var type = this._readUntil($COMMA$1);
            this._endToken([type], this._getLocation()), this._requireCharCode($COMMA$1), this._attemptCharCodeUntilFn(isNotWhitespace), this._expansionCaseStack.push(HtmlTokenType.EXPANSION_FORM_START);
            ...
            this._beginToken(HtmlTokenType.EXPANSION_CASE_EXP_END, this._getLocation()), this._requireCharCode($RBRACE$1), this._endToken([], this._getLocation()), this._attemptCharCodeUntilFn(isNotWhitespace), this._expansionCaseStack.pop();
          }, _HtmlTokenizer.prototype._consumeExpansionFormEnd = function() {
            this._beginToken(HtmlTokenType.EXPANSION_FORM_END, this._getLocation()), this._requireCharCode($RBRACE$1), this._endToken([]), this._expansionCaseStack.pop();
          }, _HtmlTokenizer.prototype._consumeText = function() {
            var start = this._getLocation();
            this._beginToken(HtmlTokenType.TEXT, start);
            var parts = [],
                interpolation = !1;
            for (this._peek === $LBRACE$1 && this._nextPeek === $LBRACE$1 ? (parts.push(this._readChar(!0)), parts.push(this._readChar(!0)), interpolation = !0) : parts.push(this._readChar(!0)); !this._isTextEnd(interpolation); )
              this._peek === $LBRACE$1 && this._nextPeek === $LBRACE$1 ? (parts.push(this._readChar(!0)), parts.push(this._readChar(!0)), interpolation = !0) : this._peek === $RBRACE$1 && this._nextPeek === $RBRACE$1 && interpolation ? (parts.push(this._readChar(!0)), parts.push(this._readChar(!0)), interpolation = !1) : parts.push(this._readChar(!0));
            ...
            nbTokens < this.tokens.length && (this.tokens = ListWrapper.slice(this.tokens, 0, nbTokens));
    ...
    var PreparsedElementType,
        TreeBuilder = function() {
          ...
          return TreeBuilder.prototype.build = function() {
          ...
          }, TreeBuilder.prototype._consumeCdata = function() {
            ...
            if (this.peek.type !== HtmlTokenType.EXPANSION_FORM_END)
              return this.errors.push(HtmlTreeError.create(null, this.peek.sourceSpan, "Invalid expansion form. Missing '}'.")), void 0;
            this._advance();
            ...
            if (tagDef.requireExtraParent(isPresent(parentEl) ? parentEl.name : null)) {
              ...
              this._addToParent(newParent), this.elementStack.push(newParent), this.elementStack.push(el);
            } else
        ...
        SelectorMatcher = function() {
          ...
          return SelectorMatcher.createNotMatcher = function(notSelectors) {
            ...
            return isBlank(nestedSelector) ? !1 : nestedSelector.match(cssSelector, matchedCallback);
          }, SelectorMatcher;
        }(),
    ...
    var PreparsedElement = function() {
      ...
      return PreparsedElement;
        ...
        ValueTransformer = function() {
          ...
          }, ValueTransformer.prototype.visitPrimitive = function(value) {
            return value;
          }, ValueTransformer.prototype.visitOther = function(value) {
            return value;
          }, ValueTransformer;
        }(),
        _ASSET_SCHEME = "asset:",
    ...
    var HOST_REG_EXP = /^(?:(?:\[([^\]]+)\])|(?:\(([^\)]+)\)))$/g,
        ...
        CompileTemplateMetadata = function() {
          function CompileTemplateMetadata(_a) {
            ...
            this.encapsulation = encapsulation, this.template = template, this.templateUrl = templateUrl, this.styles = isPresent(styles) ? styles : [], this.styleUrls = isPresent(styleUrls) ? styleUrls : [], this.animations = isPresent(animations) ? ListWrapper.flatten(animations) : [], this.ngContentSelectors = isPresent(ngContentSelectors) ? ngContentSelectors : [];
          }
        ...
        impChangeDetectorState = ChangeDetectorState,
          ...
          return ProviderElementContext.prototype.afterElement = function() {
            ...
            for (var queries,
              ...
              queries = currentEl._contentQueries.get(token), isPresent(queries) && ListWrapper.addAll(result, queries.filter(function(query) {
                return query.descendants || 1 >= distance;
    ...
    var TypeModifier,
        TemplateParseVisitor = function() {
          function TemplateParseVisitor(providerViewContext, directives, pipes, _exprParser, _schemaRegistry) {
            var _this = this;
            this.providerViewContext = providerViewContext, this._exprParser = _exprParser, this._schemaRegistry = _schemaRegistry, this.errors = [], this.directivesIndex = new Map, this.ngContentCount = 0, this.selectorMatcher = new SelectorMatcher, ListWrapper.forEachWithIndex(directives, function(directive, index) {
          ...
          return TemplateParseVisitor.prototype._reportError = function(message, sourceSpan, level) {
          ...
          }, TemplateParseVisitor.prototype.visitExpansion = function() {
            return null;
          }, TemplateParseVisitor.prototype.visitExpansionCase = function() {
            return null;
          ...
          }, TemplateParseVisitor.prototype.visitComment = function() {
            return null;
            ...
            element.attrs.forEach(function(attr) {
              ...
              hasBinding || hasTemplateBinding || (attrs.push(_this.visitAttr(attr, null)), matchableAttrs.push([attr.name, attr.value])), hasTemplateBinding && (hasInlineTemplates = !0);
            });
            ...
            return isPresent(expr) ? (this._parsePropertyAst(name, expr, sourceSpan, targetMatchableAttrs, targetProps), !0) : !1;
            ...
            directives.forEach(function(directive) {
              ...
              (isPresent(event.target) || !SetWrapper.has(allDirectiveEvents, event.name)) && _this._reportError("Event binding " + event.fullName + " not emitted by any directive on an embedded template", event.sourceSpan);
            });
        ...
        NonBindableVisitor = function() {
          ...
          }, NonBindableVisitor.prototype.visitComment = function() {
            return null;
          }, NonBindableVisitor.prototype.visitAttr = function(ast) {
            return new AttrAst(ast.name, ast.value, ast.sourceSpan);
          }, NonBindableVisitor.prototype.visitText = function(ast, parent) {
            var ngContentIndex = parent.findNgContentIndex(TEXT_CSS_SELECTOR);
            return new TextAst(ast.value, ngContentIndex, ast.sourceSpan);
          }, NonBindableVisitor.prototype.visitExpansion = function(ast) {
            return ast;
          }, NonBindableVisitor.prototype.visitExpansionCase = function(ast) {
            return ast;
          }, NonBindableVisitor;
        }(),
        ...
        CompilerConfig = function() {
          function CompilerConfig(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                _e = _b.genDebugInfo,
                genDebugInfo = void 0 === _e ? assertionsEnabled() : _e,
                _f = _b.logBindingUpdate,
                logBindingUpdate = void 0 === _f ? assertionsEnabled() : _f,
                _g = _b.useJit,
                useJit = void 0 === _g ? !0 : _g,
                _h = _b.platformDirectives,
                platformDirectives = void 0 === _h ? [] : _h,
                _j = _b.platformPipes,
                platformPipes = void 0 === _j ? [] : _j;
            this.renderTypes = renderTypes, this.defaultEncapsulation = defaultEncapsulation, this.genDebugInfo = genDebugInfo, this.logBindingUpdate = logBindingUpdate, this.useJit = useJit, this.platformDirectives = platformDirectives, this.platformPipes = platformPipes;
          }
    ...
    new ReadVarExpr(BuiltinVar.CatchError), new ReadVarExpr(BuiltinVar.CatchStack);
    var StmtModifier,
    ...
    var Statement = function() {
      ...
      return Statement.prototype.hasModifier = function(modifier) {
        ...
        ExpressionTransformer = function() {
          ...
          return ExpressionTransformer.prototype.visitReadVarExpr = function(ast) {
            return ast;
          ...
          }, ExpressionTransformer.prototype.visitLiteralExpr = function(ast) {
            return ast;
          }, ExpressionTransformer.prototype.visitExternalExpr = function(ast) {
            return ast;
          ...
          }, ExpressionTransformer.prototype.visitFunctionExpr = function(ast) {
            return ast;
          ...
          }, ExpressionTransformer.prototype.visitDeclareFunctionStmt = function(stmt) {
            return stmt;
          }, ExpressionTransformer.prototype.visitExpressionStmt = function(stmt, context) {
            return new ExpressionStatement(stmt.expr.visitExpression(this, context));
          }, ExpressionTransformer.prototype.visitReturnStmt = function(stmt, context) {
            return new ReturnStatement(stmt.value.visitExpression(this, context));
          }, ExpressionTransformer.prototype.visitDeclareClassStmt = function(stmt) {
            return stmt;
          ...
          }, ExpressionTransformer.prototype.visitCommentStmt = function(stmt) {
            return stmt;
        ...
        RecursiveExpressionVisitor = function() {
          ...
          return RecursiveExpressionVisitor.prototype.visitReadVarExpr = function(ast) {
            return ast;
          ...
          }, RecursiveExpressionVisitor.prototype.visitLiteralExpr = function(ast) {
            return ast;
          }, RecursiveExpressionVisitor.prototype.visitExternalExpr = function(ast) {
            return ast;
          ...
          }, RecursiveExpressionVisitor.prototype.visitFunctionExpr = function(ast) {
            return ast;
          ...
          }, RecursiveExpressionVisitor.prototype.visitDeclareFunctionStmt = function(stmt) {
            return stmt;
          }, RecursiveExpressionVisitor.prototype.visitExpressionStmt = function(stmt, context) {
            return stmt.expr.visitExpression(this, context), stmt;
          }, RecursiveExpressionVisitor.prototype.visitReturnStmt = function(stmt, context) {
            return stmt.value.visitExpression(this, context), stmt;
          }, RecursiveExpressionVisitor.prototype.visitDeclareClassStmt = function(stmt) {
            return stmt;
          ...
          }, RecursiveExpressionVisitor.prototype.visitCommentStmt = function(stmt) {
            return stmt;
        ...
        _ReplaceVariableTransformer = function(_super) {
          ...
          return __extends(_ReplaceVariableTransformer, _super), _ReplaceVariableTransformer.prototype.visitReadVarExpr = function(ast) {
        ...
        _VariableFinder = function(_super) {
          ...
          return __extends(_VariableFinder, _super), _VariableFinder.prototype.visitReadVarExpr = function(ast) {
        ...
        StyleSheetSourceWithImports = function() {
          function StyleSheetSourceWithImports(source, importedUrls) {
            this.source = source, this.importedUrls = importedUrls;
          }
        ...
        OfflineCompiler = function() {
          function OfflineCompiler(_directiveNormalizer, _templateParser, _styleCompiler, _viewCompiler, _outputEmitter, _xhr) {
            this._directiveNormalizer = _directiveNormalizer, this._templateParser = _templateParser, this._styleCompiler = _styleCompiler, this._viewCompiler = _viewCompiler, this._outputEmitter = _outputEmitter, this._xhr = _xhr;
          }
          return OfflineCompiler.prototype.normalizeDirectiveMetadata = function(directive) {
            ...
            var statements = [],
                exportedVars = [],
                moduleUrl = _templateModuleUrl(components[0].component);
            return components.forEach(function(componentWithDirs) {
              ...
              var hostMeta = createHostComponentMeta(compMeta.type, compMeta.selector),
                  hostViewFactoryVar = _this._compileComponent(hostMeta, [compMeta], [], statements),
                  compFactoryVar = compMeta.type.name + "NgFactory";
            ...
            }), this._codegenSourceModule(moduleUrl, statements, exportedVars);
            ...
            return this._xhr.get(stylesheetUrl).then(function(cssText) {
              var compileResult = _this._styleCompiler.compileStylesheet(stylesheetUrl, cssText, shim),
                  importedUrls = [];
              return compileResult.dependencies.forEach(function(dep) {
                importedUrls.push(dep.moduleUrl), dep.valuePlaceholder.moduleUrl = _stylesModuleUrl(dep.moduleUrl, dep.isShimmed, suffix);
              }), new StyleSheetSourceWithImports(_this._codgenStyles(stylesheetUrl, shim, suffix, compileResult), importedUrls);
            });
          }, OfflineCompiler.prototype._compileComponent = function(compMeta, directives, pipes, targetStatements) {
            var styleResult = this._styleCompiler.compileComponent(compMeta),
                parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, compMeta.type.name),
                viewResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, variable(styleResult.stylesVar), pipes);
            return ListWrapper.addAll(targetStatements, _resolveStyleStatements(compMeta.type.moduleUrl, styleResult)), ListWrapper.addAll(targetStatements, _resolveViewStatements(viewResult)), viewResult.viewFactoryVar;
        ...
        PromiseWrapper = function() {
          ...
          return PromiseWrapper.resolve = function(obj) {
            ...
            PromiseWrapper.then(PromiseWrapper.resolve(null), computation, function() {});
        ...
        StylesCompileResult = function() {
          function StylesCompileResult(statements, stylesVar, dependencies) {
            this.statements = statements, this.stylesVar = stylesVar, this.dependencies = dependencies;
          }
        ...
        StyleCompiler = function() {
          ...
          return StyleCompiler.prototype.compileComponent = function(comp) {
            ...
            for (var _this = this,
                ...
                dependencies = [],
                i = 0; i < absUrls.length; i++) {
              var identifier = new CompileIdentifierMetadata({name: getStylesVarName(null)});
              dependencies.push(new StylesCompileDependency(absUrls[i], shim, identifier)), styleExpressions.push(new ExternalExpr(identifier));
            }
    ...
    var ChangeDetectorStateEnum = function() {
      ...
      return ChangeDetectorStateEnum.fromValue = function(value) {
    ...
    var _Mode,
          ...
          return __extends(CompileElement, _super), CompileElement.createNull = function() {
            ...
            for (var queries,
              ...
              queries = currentEl._queries.get(token), isPresent(queries) && ListWrapper.addAll(result, queries.filter(function(query) {
                return query.meta.descendants || 1 >= distance;
            ...
            if (isBlank(result) && isPresent(dep.query) && (result = this._addQuery(dep.query, null).queryList), isBlank(result) && isPresent(dep.viewQuery) && (result = createQueryList(dep.viewQuery, null, "_viewQuery_" + dep.viewQuery.selectors[0].name + "_" + this.nodeIndex + "_" + this._componentConstructorViewQueryLists.length, this.view), this._componentConstructorViewQueryLists.push(result)), isPresent(dep.token)) {
              ...
              isBlank(result) && (result = this._instances.get(dep.token));
            }
            return result;
        ...
        _ValueOutputAstTransformer = function(_super) {
          ...
          }, _ValueOutputAstTransformer.prototype.visitPrimitive = function(value) {
            return literal(value);
          }, _ValueOutputAstTransformer.prototype.visitOther = function(value) {
    ...
    var _AstToIrVisitor = function() {
      ...
      return _AstToIrVisitor.prototype.visitBinary = function(ast, mode) {
      ...
      }, _AstToIrVisitor.prototype.visitQuote = function() {
        ...
        STATE_IS_NEVER_CHECKED = THIS_EXPR.prop("cdState").identical(ChangeDetectorStateEnum.NeverChecked),
        NOT_THROW_ON_CHANGES = not(DetectChangesVars.throwOnChange),
        ViewBinderVisitor = function() {
          function ViewBinderVisitor(view) {
            this.view = view, this._nodeIndex = 0;
          }
          return ViewBinderVisitor.prototype.visitBoundText = function(ast) {
            var node = this.view.nodes[this._nodeIndex++];
            return bindRenderText(ast, node, this.view), null;
          }, ViewBinderVisitor.prototype.visitText = function() {
            return this._nodeIndex++, null;
          }, ViewBinderVisitor.prototype.visitNgContent = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitElement = function(ast) {
          ...
          }, ViewBinderVisitor.prototype.visitEmbeddedTemplate = function(ast) {
            var compileElement = this.view.nodes[this._nodeIndex++],
                eventListeners = collectEventListeners(ast.outputs, ast.directives, compileElement);
          ...
          }, ViewBinderVisitor.prototype.visitAttr = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitDirective = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitEvent = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitReference = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitVariable = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitDirectiveProperty = function() {
            return null;
          }, ViewBinderVisitor.prototype.visitElementProperty = function() {
            return null;
          }, ViewBinderVisitor;
        }(),
        ...
        ViewCompileDependency = function() {
          function ViewCompileDependency(comp, factoryPlaceholder) {
            this.comp = comp, this.factoryPlaceholder = factoryPlaceholder;
          }
        ...
        ViewBuilderVisitor = function() {
          ...
          return ViewBuilderVisitor.prototype._isRootNode = function(parent) {
            ...
            if (isPresent(component)) {
              ...
              this.targetDependencies.push(new ViewCompileDependency(component, nestedComponentIdentifier)), compViewExpr = variable("compView_" + nodeIndex), compileElement.setComponentView(compViewExpr), this.view.createMethod.addStmt(compViewExpr.set(importExpr(nestedComponentIdentifier).callFn([ViewProperties.viewUtils, compileElement.injector, compileElement.appElement])).toDeclStmt());
            }
          ...
          }, ViewBuilderVisitor.prototype.visitAttr = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitDirective = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitEvent = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitReference = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitVariable = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitDirectiveProperty = function() {
            return null;
          }, ViewBuilderVisitor.prototype.visitElementProperty = function() {
            return null;
          }, ViewBuilderVisitor;
        }(),
    ...
    var XHR = function() {
      function XHR() {}
      return XHR.prototype.get = function() {
        return null;
      }, XHR;
    }(),
        DirectiveNormalizer = function() {
          function DirectiveNormalizer(_xhr, _urlResolver, _htmlParser, _config) {
            this._xhr = _xhr, this._urlResolver = _urlResolver, this._htmlParser = _htmlParser, this._config = _config;
          }
          return DirectiveNormalizer.prototype.normalizeDirective = function(directive) {
            return directive.isComponent ? this.normalizeTemplate(directive.type, directive.template).then(function(normalizedTemplate) {
              return new CompileDirectiveMetadata({
                type: directive.type,
            ...
            }) : PromiseWrapper.resolve(directive);
            ...
            var _this = this,
                rootNodesAndErrors = this._htmlParser.parse(template, directiveType.name);
            ...
            var allStyles = templateMeta.styles.concat(visitor.styles),
                allStyleAbsUrls = visitor.styleUrls.filter(isStyleUrlResolvable).map(function(url) {
                ...
                allResolvedStyles = allStyles.map(function(style) {
                  var styleWithImports = extractStyleUrls(_this._urlResolver, templateAbsUrl, style);
                  return styleWithImports.styleUrls.forEach(function(styleUrl) {
                    return allStyleAbsUrls.push(styleUrl);
                  }), styleWithImports.style;
                }),
                encapsulation = templateMeta.encapsulation;
            return isBlank(encapsulation) && (encapsulation = this._config.defaultEncapsulation), encapsulation === _angular_core.ViewEncapsulation.Emulated && 0 === allResolvedStyles.length && 0 === allStyleAbsUrls.length && (encapsulation = _angular_core.ViewEncapsulation.None), new CompileTemplateMetadata({
    ...
    var TemplatePreparseVisitor = function() {
      ...
      return TemplatePreparseVisitor.prototype.visitElement = function(ast) {
        var preparsedElement = preparseElement(ast);
      ...
      }, TemplatePreparseVisitor.prototype.visitComment = function() {
        return null;
      }, TemplatePreparseVisitor.prototype.visitAttr = function() {
        return null;
      }, TemplatePreparseVisitor.prototype.visitText = function() {
        return null;
      }, TemplatePreparseVisitor.prototype.visitExpansion = function() {
        return null;
      }, TemplatePreparseVisitor.prototype.visitExpansionCase = function() {
        return null;
        ...
        DirectiveResolver = function() {
          function DirectiveResolver(_reflector) {
            this._reflector = isPresent(_reflector) ? _reflector : reflector;
          }
          return DirectiveResolver.prototype.resolve = function(type) {
            ...
            return StringMapWrapper.forEach(propertyMetadata, function(metadata, propName) {
              metadata.forEach(function(a) {
                ...
                a instanceof _angular_core.ContentChildrenMetadata && (queries[propName] = a), a instanceof _angular_core.ViewChildrenMetadata && (queries[propName] = a), a instanceof _angular_core.ContentChildMetadata && (queries[propName] = a), a instanceof _angular_core.ViewChildMetadata && (queries[propName] = a);
              });
    ...
    DirectiveResolver.decorators = [{type: _angular_core.Injectable}], DirectiveResolver.ctorParameters = [{type: ReflectorReader}], new DirectiveResolver(reflector);
    ...
    PipeResolver.decorators = [{type: _angular_core.Injectable}], PipeResolver.ctorParameters = [{type: ReflectorReader}], new PipeResolver(reflector);
    var ViewResolver = function() {
      function ViewResolver(_reflector) {
        this._cache = new Map$1, this._reflector = isPresent(_reflector) ? _reflector : reflector;
      }
      return ViewResolver.prototype.resolve = function(component) {
        var view = this._cache.get(component);
        return isBlank(view) && (view = this._resolve(component), this._cache.set(component, view)), view;
    ...
    var CompileMetadataResolver = function() {
      function CompileMetadataResolver(_directiveResolver, _pipeResolver, _viewResolver, _config, _reflector) {
        this._directiveResolver = _directiveResolver, this._pipeResolver = _pipeResolver, this._viewResolver = _viewResolver, this._config = _config, this._directiveCache = new Map, this._pipeCache = new Map, this._anonymousTypes = new Map, this._anonymousTypeIndex = 0, this._reflector = isPresent(_reflector) ? _reflector : reflector;
      }
      return CompileMetadataResolver.prototype.sanitizeTokenName = function(token) {
        ...
        if (isBlank(meta)) {
          ...
          if (dirMeta instanceof _angular_core.ComponentMetadata) {
            ...
            templateMeta = new CompileTemplateMetadata({
              ...
              animations: animations
            }), changeDetectionStrategy = cmpMeta.changeDetection, isPresent(dirMeta.viewProviders) && (viewProviders = this.getProvidersMetadata(dirMeta.viewProviders)), moduleUrl = componentModuleUrl(this._reflector, directiveType, cmpMeta);
          }
          var providers = [];
          isPresent(dirMeta.providers) && (providers = this.getProvidersMetadata(dirMeta.providers));
          var queries = [],
              viewQueries = [];
        ...
        try {
          ...
          if (-1 !== e.message.indexOf("No Directive annotation"))
            return null;
          throw e;
        }
        ...
        var compileToken;
        return compileToken = isString(token) ? new CompileTokenMetadata({value: token}) : new CompileTokenMetadata({identifier: new CompileIdentifierMetadata({
            runtime: token,
        ...
        var compileDeps;
        return isPresent(provider.useClass) ? compileDeps = this.getDependenciesMetadata(provider.useClass, provider.dependencies) : isPresent(provider.useFactory) && (compileDeps = this.getDependenciesMetadata(provider.useFactory, provider.dependencies)), new CompileProviderMetadata({
    ...
    var _CompileValueConverter = function(_super) {
      ...
      return __extends(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value) {
        return isStaticType(value) ? new CompileIdentifierMetadata({
          name: value.name,
          moduleUrl: staticTypeModuleUrl(value)
        }) : new CompileIdentifierMetadata({runtime: value});
        ...
        EmitterVisitorContext = function() {
          ...
          return EmitterVisitorContext.createRoot = function(exportedVars) {
            ...
            return -1 !== this._exportedVars.indexOf(varName);
        ...
        StatementInterpreter = function() {
          ...
          }, StatementInterpreter.prototype.visitCommentStmt = function() {
            return null;
          ...
          }, StatementInterpreter.prototype.visitLiteralExpr = function(ast) {
            return ast.value;
          }, StatementInterpreter.prototype.visitExternalExpr = function(ast) {
        ...
        RuntimeCompiler = function() {
          ...
          return RuntimeCompiler.prototype.resolveComponent = function(component) {
            ...
            var componentType = component,
                compMeta = this._metadataResolver.getDirectiveMetadata(componentType),
                hostCacheKey = this._hostCacheKeys.get(componentType);
            if (isBlank(hostCacheKey)) {
              hostCacheKey = new Object, this._hostCacheKeys.set(componentType, hostCacheKey), assertComponent(compMeta);
              var hostMeta = createHostComponentMeta(compMeta.type, compMeta.selector);
            ...
            this._styleCache.clear(), this._compiledTemplateCache.clear(), this._compiledTemplateDone.clear(), this._hostCacheKeys.clear();
            ...
            var _this = this,
                compiledTemplate = this._compiledTemplateCache.get(cacheKey),
                done = this._compiledTemplateDone.get(cacheKey);
            return isBlank(compiledTemplate) && (compiledTemplate = new CompiledTemplate, this._compiledTemplateCache.set(cacheKey, compiledTemplate), done = PromiseWrapper.all([this._compileComponentStyles(compMeta)].concat(viewDirectives.map(function(dirMeta) {
              ...
              var normalizedViewDirMetas = stylesAndNormalizedViewDirMetas.slice(1),
                  ...
                  parsedTemplate = _this._templateParser.parse(compMeta, compMeta.template.template, normalizedViewDirMetas, pipes, compMeta.type.name),
                  childPromises = [];
              return compiledTemplate.init(_this._compileComponent(compMeta, parsedTemplate, styles, pipes, compilingComponentsPath, childPromises)), PromiseWrapper.all(childPromises).then(function() {
                return compiledTemplate;
              });
            }), this._compiledTemplateDone.set(cacheKey, done)), compiledTemplate;
            ...
            var _this = this,
                compileResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, new ExternalExpr(new CompileIdentifierMetadata({runtime: styles})), pipes);
            compileResult.dependencies.forEach(function(dep) {
              var childCompilingComponentsPath = ListWrapper.clone(compilingComponentsPath),
                  childCacheKey = dep.comp.type.runtime,
                  ...
                  childIsRecursive = ListWrapper.contains(childCompilingComponentsPath, childCacheKey);
              ...
              dep.factoryPlaceholder.runtime = childComp.proxyViewFactory, dep.factoryPlaceholder.name = "viewFactory_" + dep.comp.type.name, childIsRecursive || childPromises.push(_this._compiledTemplateDone.get(childCacheKey));
            });
            var factory;
            return factory = IS_DART || !this._genConfig.useJit ? interpretStatements(compileResult.statements, compileResult.viewFactoryVar, new InterpretiveAppViewInstanceFactory) : jitStatements(compMeta.type.name + ".template.js", compileResult.statements, compileResult.viewFactoryVar);
            ...
            var _this = this,
                promises = result.dependencies.map(function(dep) {
                  return _this._loadStylesheetDep(dep);
                });
            return PromiseWrapper.all(promises).then(function(cssTexts) {
              for (var nestedCompileResultPromises = [],
                  i = 0; i < result.dependencies.length; i++) {
                var dep = result.dependencies[i],
                    cssText = cssTexts[i],
                    nestedCompileResult = _this._styleCompiler.compileStylesheet(dep.moduleUrl, cssText, dep.isShimmed);
              ...
              for (var i = 0; i < result.dependencies.length; i++) {
                var dep = result.dependencies[i];
                dep.valuePlaceholder.runtime = nestedStylesArr[i], dep.valuePlaceholder.name = "importedStyles" + i;
              }
              return IS_DART || !_this._genConfig.useJit ? interpretStatements(result.statements, result.stylesVar, new InterpretiveAppViewInstanceFactory) : jitStatements(sourceUrl + ".css.js", result.statements, result.stylesVar);
            });
          }, RuntimeCompiler.prototype._loadStylesheetDep = function(dep) {
            var cacheKey = "" + dep.moduleUrl + (dep.isShimmed ? ".shim" : ""),
                cssTextPromise = this._styleCache.get(cacheKey);
            return isBlank(cssTextPromise) && (cssTextPromise = this._xhr.get(dep.moduleUrl), this._styleCache.set(cacheKey, cssTextPromise)), cssTextPromise;
          }, RuntimeCompiler;
        }();
    RuntimeCompiler.decorators = [{type: _angular_core.Injectable}], RuntimeCompiler.ctorParameters = [{type: CompileMetadataResolver}, {type: DirectiveNormalizer}, {type: TemplateParser}, {type: StyleCompiler}, {type: ViewCompiler}, {type: XHR}, {type: CompilerConfig}];
    var CompiledTemplate = function() {
      function CompiledTemplate() {
        var _this = this;
        this.viewFactory = null, this.proxyViewFactory = function(viewUtils, childInjector, contextEl) {
          return _this.viewFactory(viewUtils, childInjector, contextEl);
        };
      }
      return CompiledTemplate.prototype.init = function(viewFactory) {
      ...
      useExisting: DomElementSchemaRegistry
        ...
        _StringifyVisitor = function() {
          function _StringifyVisitor(_parser) {
            this._parser = _parser, this._index = 0;
          }
          return _StringifyVisitor.prototype.visitElement = function(ast) {
            var name = this._index++,
                children = this._join(htmlVisitAll(this, ast.children), "");
            return '<ph name="e' + name + '">' + children + "</ph>";
          }, _StringifyVisitor.prototype.visitAttr = function() {
            return null;
          }, _StringifyVisitor.prototype.visitText = function(ast) {
            var index = this._index++,
                noInterpolation = removeInterpolation(ast.value, ast.sourceSpan, this._parser);
            return noInterpolation != ast.value ? '<ph name="t' + index + '">' + noInterpolation + "</ph>" : ast.value;
          }, _StringifyVisitor.prototype.visitComment = function() {
            return "";
          }, _StringifyVisitor.prototype.visitExpansion = function() {
            return null;
          }, _StringifyVisitor.prototype.visitExpansionCase = function() {
            return null;
        ...
        _Expander = function() {
          function _Expander() {
            this.expanded = !1, this.errors = [];
          }
          return _Expander.prototype.visitElement = function(ast) {
          ...
          }, _Expander.prototype.visitAttr = function(ast) {
            return ast;
          }, _Expander.prototype.visitText = function(ast) {
            return ast;
          }, _Expander.prototype.visitComment = function(ast) {
            return ast;
          }, _Expander.prototype.visitExpansion = function(ast) {
            return this.expanded = !0, "plural" == ast.type ? _expandPluralForm(ast, this.errors) : _expandDefaultForm(ast);
          }, _Expander.prototype.visitExpansionCase = function() {
        ...
        I18nHtmlParser = function() {
          ...
          return I18nHtmlParser.prototype.parse = function(sourceContent, sourceUrl, parseExpansionForms) {
            void 0 === parseExpansionForms && (parseExpansionForms = !1), this.errors = [];
            ...
            return el.attrs.forEach(function(attr) {
              if (!attr.name.startsWith(I18N_ATTR_PREFIX) && attr.name != I18N_ATTR) {
                ...
                if (0 == i18ns.length) {
                  if (-1 == implicitAttrs.indexOf(attr.name))
                    return res.push(attr), void 0;
        ...
        _CreateNodeMapping = function() {
          ...
          return _CreateNodeMapping.prototype.visitElement = function(ast) {
            return this.mapping.push(ast), htmlVisitAll(this, ast.children), null;
          }, _CreateNodeMapping.prototype.visitAttr = function() {
            return null;
          }, _CreateNodeMapping.prototype.visitText = function(ast) {
            return this.mapping.push(ast), null;
          }, _CreateNodeMapping.prototype.visitExpansion = function() {
            return null;
          }, _CreateNodeMapping.prototype.visitExpansionCase = function() {
            return null;
          }, _CreateNodeMapping.prototype.visitComment = function() {
            return "";
          }, _CreateNodeMapping;
        }(),
        ...
        MessageExtractor = function() {
          ...
          return MessageExtractor.prototype.extract = function(template, sourceUrl) {
            this._messages = [], this._errors = [];
            var res = this._htmlParser.parse(template, sourceUrl, !0);
            if (res.errors.length > 0)
              return new ExtractionResult([], res.errors);
            var expanded = expandNodes(res.rootNodes);
            return this._recurse(expanded.nodes), new ExtractionResult(this._messages, this._errors.concat(expanded.errors));
          }, MessageExtractor.prototype._extractMessagesFromPart = function(part) {
            part.hasI18n ? (this._messages.push(part.createMessage(this._parser)), this._recurseToExtractMessagesFromAttributes(part.children)) : this._recurse(part.children), isPresent(part.rootElement) && this._extractMessagesFromAttributes(part.rootElement);
            ...
            nodes.forEach(function(n) {
              n instanceof HtmlElementAst && (_this._extractMessagesFromAttributes(n), _this._recurseToExtractMessagesFromAttributes(n.children));
            });
            ...
            p.attrs.filter(function(attr) {
              ...
              try {
                explicitAttrs.push(attr.name.substring(I18N_ATTR_PREFIX.length)), _this._messages.push(messageFromI18nAttribute(_this._parser, p, attr));
              } catch (e) {
                if (!(e instanceof I18nError))
                  throw e;
              ...
              return -1 == explicitAttrs.indexOf(attr.name);
            }).filter(function(attr) {
